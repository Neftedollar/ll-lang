// FILE: 07-text-processing.fs
module Examples.TextProcessing
...
let countDigits s =
    (((listFold (fun acc -> (fun c -> (if (charIsDigit c) then (acc + 1L) else acc)))) 0L) (strChars s))

[<EntryPoint>]
let main (argv: string[]) =
    (printfn (intToStr (countDigits "abc123def45")))
    0