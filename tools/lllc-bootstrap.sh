...
filter_self_compile_output() {
  awk 'p || /^\{/ || /^module / || /^\/\/ Generated by lllc/ || /^# Generated by lllc/ || /^; ll-lang LLVM IR output/ { p=1; print }'
}

filter_self_json_output() {
  awk 'p || /^\{/ { p=1; print }'
}
...
if [[ "${1:-}" == "self" ]]; then
  case "${2:-}" in
    compile)
      set +e
      out="$("$BOOTSTRAP_BIN" "$@" 2> >(cat >&2))"
      status=$?
      set -e
      printf '%s\n' "$out" | filter_self_compile_output
      exit "$status"
      ;;
    check|next)
      set +e
