...
and __test_main_Parser () =
    (let t1 = ((IntLit 42L) :: (Eof :: [])) in (let _ = (((parserCheckExpr "1 int literal") t1) "EInt 42") in (let t2 = ((Ident "x") :: (Eof :: [])) in (let _ = (((parserCheckExpr "2 variable") t2) "EVar x") in (let t3 = ((Ident "f") :: ((Ident "x") :: (Eof :: []))) in (let _ = (((parserCheckExpr "3 application") t3) "EApp(EVar f EVar x)") in (let t4 = ((StrLit "hello") :: (Eof :: [])) in (let _ = (((parserCheckExpr "4 string literal") t4) "EStr hello") in (let t5 = (KwTrue :: (Eof :: [])) in (let _ = (((parserCheckExpr "5 bool true") t5) "EBool true") in (let t6 = (KwFalse :: (Eof :: [])) in (let _ = (((parserCheckExpr "6 bool false") t6) "EBool false") in (let t7 = ((IntLit 1L) :: (Plus :: ((IntLit 2L) :: (Eof :: [])))) in (let _ = (((parserCheckExpr "7 addition") t7) "EBinOp(+ EInt 1 EInt 2)") in (let t8 = ((Ident "x") :: (EqEq :: ((IntLit 42L) :: (Eof :: [])))) in (let _ = (((parserCheckExpr "8 comparison") t8) "EBinOp(== EVar x EInt 42)") in (let t9 = (KwIf :: (KwTrue :: (Newline :: ((IntLit 1L) :: (Newline :: (KwElse :: ((IntLit 2L) :: (Eof :: [])))))))) in (let _ = (((parserCheckExpr "9 if-else") t9) "EIf(EBool true EInt 1 EInt 2)") in (let t10 = ((Ident "x") :: (Eq :: ((IntLit 42L) :: (Newline :: ((Ident "x") :: (Eof :: [])))))) in (let _ = (((parserCheckExpr "10 let") t10) "ELet x=(EInt 42) in (EVar x)") in (let t11 = (Backslash :: ((Ident "x") :: (Dot :: ((Ident "x") :: (Eof :: []))))) in (let _ = (((parserCheckExpr "11 lambda") t11) "ELam x.EVar x") in (let t12 = ((TypeId "Some") :: ((IntLit 42L) :: (Eof :: []))) in (let _ = (((parserCheckExpr "12 constructor app") t12) "EApp(ECon Some EInt 42)") in (let t13 = (LBrack :: (RBrack :: (Eof :: []))) in (let _ = (((parserCheckExpr "13 empty list") t13) "ENil") in (let t14 = ((Ident "x") :: (ColonColon :: ((Ident "xs") :: (Eof :: [])))) in (let _ = (((parserCheckExpr "14 cons") t14) "ECons(EVar x EVar xs)") in (let t15 = (KwMatch :: ((Ident "x") :: (Newline :: (Bar :: ((TypeId "Some") :: ((Ident "v") :: (Arrow :: ((Ident "v") :: (Newline :: (Bar :: ((TypeId "None") :: (Arrow :: ((IntLit 0L) :: (Eof :: [])))))))))))))) in (let _ = (((parserCheckExpr "15 match") t15) "EMatch(EVar x...)") in (let t16 = ((Ident "add") :: (LParen :: ((Ident "a") :: ((TypeId "Int") :: (RParen :: (LParen :: ((Ident "b") :: ((TypeId "Int") :: (RParen :: (Eq :: ((Ident "a") :: (Plus :: ((Ident "b") :: (Eof :: [])))))))))))))) in (let _ = (((parserCheckDecl "16 fn decl") t16) "DFn add body=(EBinOp(+ EVar a EVar b))") in (let t17 = ((TypeId "Maybe") :: ((TypeId "A") :: (Eq :: ((TypeId "Some") :: ((TypeId "A") :: (Bar :: ((TypeId "None") :: (Eof :: [])))))))) in (let _ = (((parserCheckDecl "17 type decl") t17) "DType Maybe") in (let t18 = (KwImport :: ((TypeId "Std") :: (Dot :: ((TypeId "Lexer") :: (Eof :: []))))) in (let _ = (((parserCheckDecl "18 import") t18) "DImport Std.Lexer") in (let t19 = (KwLet :: ((Ident "x") :: (Eq :: ((IntLit 99L) :: (Eof :: []))))) in (let _ = (((parserCheckDecl "19 let decl") t19) "DLet x=(EInt 99)") in (let t20 = (KwModule :: ((TypeId "Test") :: (Newline :: ((TypeId "Foo") :: (Eq :: ((TypeId "Bar") :: (Newline :: ((Ident "f") :: (LParen :: ((Ident "x") :: ((TypeId "Int") :: (RParen :: (Eq :: ((Ident "x") :: (Eof :: []))))))))))))))) in (let _ = (((parserCheckModule "20 module") t20) 2L) in (let t21 = ((Ident "f") :: ((Ident "x") :: ((Ident "y") :: (Eof :: [])))) in (let _ = (((parserCheckExpr "21 nested app") t21) "EApp(EApp(EVar f EVar x) EVar y)") in (let t22 = ((IntLit 2L) :: (Star :: ((IntLit 3L) :: (Eof :: [])))) in (let _ = (((parserCheckExpr "22 multiply") t22) "EBinOp(* EInt 2 EInt 3)") in (let t23 = ((IntLit 1L) :: (Plus :: ((IntLit 2L) :: (Star :: ((IntLit 3L) :: (Eof :: [])))))) in (let _ = (((parserCheckExpr "23 precedence") t23) "EBinOp(+ EInt 1 EBinOp(* EInt 2 EInt 3))") in (let t24 = (LParen :: ((Ident "x") :: (RParen :: (Eof :: [])))) in (let _ = (((parserCheckExpr "24 paren grouping") t24) "EVar x") in (let t25 = (LParen :: ((Ident "a") :: (Comma :: ((Ident "b") :: (RParen :: (Eof :: [])))))) in (let _ = (((parserCheckExpr "25 tuple") t25) "ETuple(EVar a EVar b)") in 0L))))))))))))))))))))))))))))))))))))))))))))))))))