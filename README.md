...
Working end-to-end compiler with a **401-test** suite, written in F# / .NET 10. All 7 compiler phases green: lexer → parser → elaborator → Hindley-Milner inference → F# codegen → `lllc` CLI → stdlib (~50 builtins).
...
| **3-stage bootstrap compiler** | **parser + elaborator + minimal HM type checker in one program** (E001 arithmetic / if-branch mismatches, E002 unbound-var, E003 non-exhaustive match all produced by a single ll-lang file over a shared AST) | ✅ | [`20-bootstrap-compiler.lll`](spec/examples/valid/20-bootstrap-compiler.lll) |
...
| 7.9b+ | `bootstrap/compiler.lll` full end-to-end assembly (add codegen stage) | 🚧 |
