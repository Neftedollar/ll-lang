...
| **Codegen (TExpr → F#)** | `TExpr` / `TDecl` / `Pat` AST + `showTExpr` / `showDecl` emitting F# source for TEInt / TEStr / TEVar / TEAdd / TEApp / TELet + **TELam + TEIf + TEMatch** / TDFn | ✅ | [`19-codegen-real.lll`](spec/examples/valid/19-codegen-real.lll) |
...
| **7.8a + 7.8b** | **Codegen slices A + B in ll-lang** — `TExpr` / `TDecl` / `Pat` AST + `showTExpr` / `showDecl` emitting F# source for TEInt / TEStr / TEVar / TEAdd / TEApp / TELet / **TELam + TEIf + TEMatch** (single-line match form) / TDFn; `19-codegen-real.lll`, 341 lines. All four compiler stages now have ll-lang representations covering every basic control-flow shape | ✅ |
| 7.8c+ | Codegen TDType sum-type emission, F# prelude block, `let rec`/`and` grouping, `[<EntryPoint>]` main | 🚧 |
