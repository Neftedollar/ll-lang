...
and classifyIdent s = (match s with | "module" -> TKwModule | "type" -> TKwType | "fn" -> TKwFn | "if" -> TKwIf | "else" -> TKwElse | "let" -> TKwLet | "in" -> TKwIn | "match" -> TKwMatch | "tag" -> TKwTag | "import" -> TKwImport | "export" -> TKwExport | _ -> ((classifyIdentByHead s) (strChars s)))
