...
/var/folders/_3/c83rr8ys2qq_tb9cgbthkhjm0000gn/T/tmpsp7tNU.tmp.fsx(840,71): warning FS0026: This rule will never be matched
...
and stdlibNames _ignored = ("strConcat" :: ("strLen" :: ("strChars" :: ("strFromChars" :: ("strToInt" :: ("print" :: ("printfn" :: ("listMap" :: ("listLen" :: ("listAppend" :: ("listIsEmpty" :: ("listFold" :: ("listReverse" :: ("readFile" :: ("true" :: ("false" :: ("charToInt" :: ("charIsDigit" :: ("charIsSpace" :: ("intToStr" :: []))))))))))))))))))))
...
and showErrs errs = (if (listIsEmpty errs) then "no errors" else (((listFold (fun acc -> (fun s -> (if ((strLen acc) = 0L) then s else ((strConcat ((strConcat acc) "\n")) s))))) "") errs))
and emitPat p = (match p with | (PInt n) -> ((strConcat (intToStr n)) "L") | (PStr s) -> ((strConcat ((strConcat "\"") s)) "\"") | (PVar x) -> x | (PWild) -> "_" | (PNil) -> "[]" | (PCons h t) -> ((strConcat "(") ((strConcat (emitPat h)) ((strConcat " :: ") ((strConcat (emitPat t)) ")")))) | (PCon name args) -> ((strConcat "(") ((strConcat name) ((strConcat (emitPatArgs args)) ")"))))
...
and encodeStrEscape c = (if (c = '\n') then "\\n" else (if (c = '\t') then "\\t" else (if (c = '\\') then "\\\\" else (if (c = '"') then "\\\"" else (strFromChars (c :: []))))))
...
and emitIfExpr c t el = ((strConcat "(if ") ((strConcat (emitExpr c)) ((strConcat " then ") ((strConcat (emitExpr t)) ((strConcat " else ") ((strConcat (emitExpr el)) ")"))))))
...
and emitArmsCons p prest bs = (match bs with | (b :: brest) -> (let head = ((strConcat "| ") ((strConcat (emitPat p)) ((strConcat " -> ") (emitExpr b)))) in (let rest = ((emitArms prest) brest) in (if ((strLen rest) = 0L) then head else ((strConcat head) ((strConcat " ") rest))))) | _ -> "")
and emitTypeParam p = ((strConcat "'") p)
and emitTypeArgName n = (if (n = "Int") then "int64" else (if (n = "Str") then "string" else (if (n = "Bool") then "bool" else n)))
...
and emitIsMainFn name params = (match params with | (_ :: _) -> false | _ -> (if (name = "main") then true else false))
...
and emitDeclStandaloneFn fd = (match fd with | (MkFn name params _ body) -> (let pnames = (emitParamNames params) in (if ((emitIsMainFn name) params) then (emitMainDecl body) else (((emitFnPlain name) pnames) body))))
and emitDeclFnFirst d = (match d with | (DFn fd) -> (emitDeclFnFirstUnwrap fd) | _ -> "")
and emitDeclFnFirstUnwrap fd = (match fd with | (MkFn name params _ body) -> (let pnames = (emitParamNames params) in (((emitFnFirst name) pnames) body)))
and emitDeclFnCont d = (match d with | (DFn fd) -> (emitDeclFnContUnwrap fd) | _ -> "")
and emitDeclFnContUnwrap fd = (match fd with | (MkFn name params _ body) -> (let pnames = (emitParamNames params) in (((emitFnCont name) pnames) body)))
and emitFnGroup fns = (match fns with | (d :: rest) -> ((emitFnGroupCons d) rest) | _ -> "")
and emitFnGroupCons first rest = (let head = (emitDeclFnFirst first) in (let tails = ((listMap emitDeclFnCont) rest) in (let tailStr = (emitJoinLines tails) in (if ((strLen tailStr) = 0L) then head else ((strConcat head) ((strConcat "\n") tailStr))))))
and emitJoinLines xs = (((listFold (fun acc -> (fun s -> (if ((strLen acc) = 0L) then s else ((strConcat ((strConcat acc) "\n")) s))))) "") xs)
and emitJoinBlocks xs = (((listFold (fun acc -> (fun s -> (if ((strLen acc) = 0L) then s else ((strConcat ((strConcat acc) "\n\n")) s))))) "") xs)
and emitAppendBlock block acc = (if ((strLen acc) = 0L) then block else ((strConcat acc) ((strConcat "\n\n") block)))
...
and emitStepFn d name params rest pending acc = (if ((emitIsMainFn name) params) then ((((emitStepMain d) rest) pending) acc) else (((emitGroupLoop rest) (d :: pending)) acc))
...
and emitStepFnUnwrap d fd rest pending acc = (match fd with | (MkFn name params _ _) -> ((((((emitStepFn d) name) params) rest) pending) acc))
...
and emitPrelude _ignored = (let l1 = "// --- ll-lang stdlib prelude (auto-generated) ---" in (let l2 = "let listMap f xs = List.map f xs" in (let l3 = "let listLen (xs: 'a list) : int64 = int64 (List.length xs)" in (let l4 = "let strLen (s: string) : int64 = int64 s.Length" in (let l5 = "let strConcat (a: string) (b: string) = a + b" in (let l6 = "let print (s: string) = System.Console.Write(s)" in (let l7 = "// --- end prelude ---" in (let lines = (l1 :: (l2 :: (l3 :: (l4 :: (l5 :: (l6 :: (l7 :: []))))))) in (emitJoinLines lines)))))))))
and emitModuleBody path decls = (let header = ((strConcat "module ") path) in (let prelude = (emitPrelude 0L) in (let declBlock = (emitGroupedDecls decls) in (let parts = (header :: (prelude :: (declBlock :: []))) in (emitJoinBlocks parts)))))
and emitModule m = (match m with | (MkModule path decls) -> ((emitModuleBody path) decls))

[<EntryPoint>]
let main (argv: string[]) =
    (let src = (readFile "spec/examples/valid/20a-bootstrap-input.lll") in (let toks = (tokenize src) in (let mod = (parseModule toks) in (match mod with | (MkModule _ decls) -> (let errs = (elaborate decls) in (if (listIsEmpty errs) then (printfn (emitModule mod)) else (printfn (showErrs errs))))))))
    0
