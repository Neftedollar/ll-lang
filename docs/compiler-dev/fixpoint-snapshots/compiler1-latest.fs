...
and envHas env name = (match env with | (MkEnv xs) -> (((listFold (fun acc -> (fun x -> (if acc then true else (if (x = name) then true else false))))) false) xs))
...
and emitPatArgsTuple ps = (((listFold (fun acc -> (fun p -> (if ((strLen acc) = 0L) then (emitPat p) else ((strConcat acc) ((strConcat ", ") (emitPat p))))))) "") ps)
...
and emitArgsTuple args = (((listFold (fun acc -> (fun a -> (if ((strLen acc) = 0L) then a else ((strConcat acc) ((strConcat ", ") a)))))) "") args)
and emitApp f a = (let (head, args) = ((gatherAppHead f) (a :: [])) in (match head with | (EVar name) -> (if (isConstrName name) then (match args with | (_ :: (_ :: _)) -> (let strs = ((listMap emitExpr) args) in ((strConcat "(") ((strConcat name) ((strConcat " (") ((strConcat (emitArgsTuple strs)) "))"))))) | _ -> ((strConcat "(") ((strConcat (emitExpr f)) ((strConcat " ") ((strConcat (emitExpr a)) ")"))))) else ((strConcat "(") ((strConcat (emitExpr f)) ((strConcat " ") ((strConcat (emitExpr a)) ")"))))) | _ -> ((strConcat "(") ((strConcat (emitExpr f)) ((strConcat " ") ((strConcat (emitExpr a)) ")"))))))
...
and emitTypeArgName n = (if (n = "Int") then "int64" else (if (n = "Str") then "string" else (if (n = "Bool") then "bool" else (if (n = "Char") then "char" else n))))
...
and emitIsMainFn name parms = (match parms with | (_ :: _) -> false | _ -> (if (name = "main") then true else false))
...
and emitDeclStandaloneFn fd = (match fd with | (MkFn (name, parms, _, body)) -> (let pnames = (emitParamNames parms) in (if ((emitIsMainFn name) parms) then (emitMainDecl body) else (((emitFnPlain name) pnames) body))))
...
and emitStepFn d name parms rest pending acc = (if ((emitIsMainFn name) parms) then ((((emitStepMain d) rest) pending) acc) else (((emitGroupLoop rest) (d :: pending)) acc))
